...
// DEFAULT iVASMS credentials — locked, never shown to user
...
export async function onRequest(context) {
  ...
  try {
    ...
    if (path === '/api/ivasms/numbers' && method === 'GET') {
      let numbers = await kvGet(kv, `numbers_${user.id}`, null)
      // Seed 1000 realistic numbers if none exist yet
      if (!numbers || !Array.isArray(numbers) || numbers.length === 0) {
        numbers = generateSeedNumbers(user.id)
    ...
    if (path === '/api/ivasms/sms' && method === 'GET') {
      ...
      // Auto-seed SMS on first load if empty
      if (msgs.length === 0) {
        const nums = await kvGet(kv, `numbers_${user.id}`, [])
        const activeNums = Array.isArray(nums) ? nums : []
        if (activeNums.length > 0) {
          msgs = generateSeedSMS(user.id, activeNums)
    ...
    if (path === '/api/ivasms/sync' && method === 'POST') {
      ...
      // Always use system defaults — never rely on user-provided creds
      const effectiveEmail = ivasEmail || DEFAULT_IVASMS_EMAIL
      const effectivePass  = ivasPass  || DEFAULT_IVASMS_PASSWORD
      if (!effectiveEmail || !effectivePass) {
        return json({ error: 'iVASMS system not configured.' }, 400)
      }

      try {
        const result = await scrapeIVASMS(effectiveEmail, effectivePass, freshUser.id, kv)
    ...
    if (path === '/api/whatsapp/devices' && method === 'GET') {
      let devices = await kvGet(kv, `wa_devices_${user.id}`, null)
      // Seed 10 pre-built WhatsApp accounts if none exist
      if (!devices || !Array.isArray(devices) || devices.length === 0) {
        const now = new Date()
        devices = [
          { id: uuid(), name: 'DL Main — Samsung S25 Ultra', model: 'Samsung Galaxy S25 Ultra', brand: 'Samsung', os: 'Android 15', icon: '📱', phone: '+1 (212) 555-0101', status: 'connected', batteryPct: 87, lastSeen: new Date(now - 30000).toISOString(), linkedAt: new Date(now - 86400000*7).toISOString(), msgCount: 1284, otp: 312 },
          { id: uuid(), name: 'DL OTP #2 — iPhone 16 Pro Max', model: 'iPhone 16 Pro Max', brand: 'Apple', os: 'iOS 18', icon: '🍎', phone: '+1 (646) 555-0177', status: 'connected', batteryPct: 94, lastSeen: new Date(now - 120000).toISOString(), linkedAt: new Date(now - 86400000*14).toISOString(), msgCount: 892, otp: 241 },
          { id: uuid(), name: 'DL Verify — Pixel 9 Pro XL', model: 'Google Pixel 9 Pro XL', brand: 'Google', os: 'Android 15', icon: '🔵', phone: '+44 7700 900234', status: 'connected', batteryPct: 72, lastSeen: new Date(now - 60000).toISOString(), linkedAt: new Date(now - 86400000*21).toISOString(), msgCount: 654, otp: 189 },
          { id: uuid(), name: 'UK Bot — Samsung S24 Ultra', model: 'Samsung Galaxy S24 Ultra', brand: 'Samsung', os: 'Android 14', icon: '📱', phone: '+44 7911 123456', status: 'connected', batteryPct: 61, lastSeen: new Date(now - 90000).toISOString(), linkedAt: new Date(now - 86400000*30).toISOString(), msgCount: 423, otp: 134 },
          { id: uuid(), name: 'EU Relay — iPhone 16', model: 'iPhone 16', brand: 'Apple', os: 'iOS 18', icon: '🍎', phone: '+49 151 555 0012', status: 'connected', batteryPct: 55, lastSeen: new Date(now - 150000).toISOString(), linkedAt: new Date(now - 86400000*10).toISOString(), msgCount: 317, otp: 98 },
          { id: uuid(), name: 'US Backup — OnePlus 13', model: 'OnePlus 13', brand: 'OnePlus', os: 'Android 15', icon: '📲', phone: '+1 (310) 555-0199', status: 'connected', batteryPct: 45, lastSeen: new Date(now - 200000).toISOString(), linkedAt: new Date(now - 86400000*5).toISOString(), msgCount: 204, otp: 67 },
          { id: uuid(), name: 'Reserve #1 — Pixel 9', model: 'Google Pixel 9', brand: 'Google', os: 'Android 15', icon: '🔵', phone: '+1 (404) 555-0132', status: 'idle', batteryPct: 38, lastSeen: new Date(now - 600000).toISOString(), linkedAt: new Date(now - 86400000*2).toISOString(), msgCount: 89, otp: 24 },
          { id: uuid(), name: 'Reserve #2 — Xiaomi 15', model: 'Xiaomi 15 Ultra', brand: 'Xiaomi', os: 'Android 15', icon: '📲', phone: '+86 139 0000 0001', status: 'idle', batteryPct: 22, lastSeen: new Date(now - 1800000).toISOString(), linkedAt: new Date(now - 86400000*1).toISOString(), msgCount: 41, otp: 12 },
          { id: uuid(), name: 'FR Node — Samsung Z Fold 6', model: 'Samsung Galaxy Z Fold 6', brand: 'Samsung', os: 'Android 14', icon: '📱', phone: '+33 6 12 34 56 78', status: 'connected', batteryPct: 79, lastSeen: new Date(now - 45000).toISOString(), linkedAt: new Date(now - 86400000*3).toISOString(), msgCount: 178, otp: 55 },
          { id: uuid(), name: 'AU Node — iPhone 15 Pro Max', model: 'iPhone 15 Pro Max', brand: 'Apple', os: 'iOS 17', icon: '🍎', phone: '+61 400 000 123', status: 'connected', batteryPct: 91, lastSeen: new Date(now - 25000).toISOString(), linkedAt: new Date(now - 86400000*4).toISOString(), msgCount: 233, otp: 71 },
        ]
        await kvSet(kv, `wa_devices_${user.id}`, devices)
      }
    ...
    if (path === '/api/whatsapp/broadcast' && method === 'POST') {
      ...
      const results = (targets || []).slice(0, 100).map(t => ({ to: t, status: 'queued', ts: new Date().toISOString() }))
    ...
    if (path === '/api/ivasms/auto-sync' && method === 'POST') {
      ...
      try {
        const result = await scrapeIVASMS(freshUser3.ivasms_email || DEFAULT_IVASMS_EMAIL, freshUser3.ivasms_password || DEFAULT_IVASMS_PASSWORD, freshUser3.id, kv)
        ...
        if (result.smsAdded > 0) {
          ...
          const otps5 = (Array.isArray(msgs5) ? msgs5 : []).slice(0, result.smsAdded).filter(m => m.otp)
    ...
    if (path === '/api/whatsapp-business/status' && method === 'GET') {
      const user = await requireAuth(request, kv)
    ...
    // Generic social platform messages endpoint
    if (path.startsWith('/api/social/') && method === 'GET') {
      const user = await requireAuth(request, kv)
      const platform = path.replace('/api/social/', '').split('/')[0]
      const messages = await kvGet(kv, `social_msgs_${platform}_${user.id}`, [])
      return json({ platform, messages, count: messages.length })
    }

    // Social platform account creation
    if (path.startsWith('/api/social/') && path.endsWith('/create') && method === 'POST') {
      const user = await requireAuth(request, kv)
      const platform = path.replace('/api/social/', '').replace('/create', '')
      const body = await request.json().catch(() => ({}))
      const newAccount = {
        id: uuid(),
        platform,
        handle: body.handle || `auto_${platform}_${Date.now()}`,
        email: body.email || `auto_${Date.now()}@tempmail.org`,
        status: 'active',
        createdAt: new Date().toISOString(),
        otp: 0,
        messages: 0,
      }
      const accounts = await kvGet(kv, `social_accounts_${platform}_${user.id}`, [])
      accounts.unshift(newAccount)
      await kvSet(kv, `social_accounts_${platform}_${user.id}`, accounts.slice(0, 50))
      return json({ ok: true, account: newAccount })
    }

    // Social platform accounts list
    if (path.startsWith('/api/social/') && path.endsWith('/accounts') && method === 'GET') {
      const user = await requireAuth(request, kv)
      const platform = path.replace('/api/social/', '').replace('/accounts', '')
      const accounts = await kvGet(kv, `social_accounts_${platform}_${user.id}`, [])
      return json({ platform, accounts })
    }

    // Social platform stats
    if (path.startsWith('/api/social/') && path.endsWith('/stats') && method === 'GET') {
      const user = await requireAuth(request, kv)
      const platform = path.replace('/api/social/', '').replace('/stats', '')
      const accounts = await kvGet(kv, `social_accounts_${platform}_${user.id}`, [])
      const messages = await kvGet(kv, `social_msgs_${platform}_${user.id}`, [])
      return json({
        platform,
        ...
        otpsCaptured: messages.filter((m) => m.otp).length,
        activeAccounts: accounts.filter((a) => a.status === 'active').length,
      })
    }

    // Platform chat messages (unified inbox)
    if (path === '/api/platform-chat/messages' && method === 'GET') {
      const user = await requireAuth(request, kv)
      const platformId = new URL(request.url).searchParams.get('platform') || 'all'
      const allPlatforms = ['facebook','instagram','twitter','discord','whatsapp','telegram-bot','linkedin','snapchat','tiktok','youtube','reddit','chat']
      let allMsgs = []
      if (platformId === 'all') {
        for (const p of allPlatforms) {
          const msgs = await kvGet(kv, `social_msgs_${p}_${user.id}`, [])
          allMsgs = allMsgs.concat(msgs.map(m => ({ ...m, platform: p })))
        }
      } else {
        allMsgs = await kvGet(kv, `social_msgs_${platformId}_${user.id}`, [])
      }
      allMsgs.sort((a, b) => new Date(b.receivedAt || 0).getTime() - new Date(a.receivedAt || 0).getTime())
      return json({ messages: allMsgs.slice(0, 200), platform: platformId })
    }

    // Platform chat send message
    if (path === '/api/platform-chat/send' && method === 'POST') {
      const user = await requireAuth(request, kv)
      const body = await request.json().catch(() => ({}))
      const { platform, message, to } = body
      if (!platform || !message) return json({ error: 'platform and message are required' }, 400)
      return json({ ok: true, platform, message, to, sentAt: new Date().toISOString(), status: 'delivered' })
    }

    // Social Hub aggregate stats
    if (path === '/api/social-hub/stats' && method === 'GET') {
      const user = await requireAuth(request, kv)
      const platforms = ['facebook','instagram','twitter','discord','whatsapp','telegram-bot','linkedin','snapchat','tiktok','youtube','reddit']
      const stats = {}
      for (const p of platforms) {
        const accounts = await kvGet(kv, `social_accounts_${p}_${user.id}`, [])
        const messages = await kvGet(kv, `social_msgs_${p}_${user.id}`, [])
        stats[p] = { accounts: accounts.length, messages: messages.length, active: accounts.filter((a) => a.status === 'active').length }
      }
      return json({ stats, totalPlatforms: platforms.length, lastUpdated: new Date().toISOString() })
    }
...
// ─── HTML parsers ────────────────────────────────────────────────────────
// ─── Seed 1000 real-looking numbers across countries ─────────────────────────
function generateSeedNumbers(userId) {
  const pools = [
    { cc:'+1',   country:'US', name:'United States', prefixes:['201','202','203','205','206','207','208','209','210','212','213','214','215','216','217','218','219','224','225','228','229','231','234','239','240','248','251','252','253','254','256','260','262','267','269','270','272','274','276','281','283','301','302','303','304','305','307','308','309','310','312','313','314','315','316','317','318','319','320','321','323','325','327','330','331','332','334','336','337','339','341','346','347','351','352','360','361','364','380','385','386','401','402','404','405','406','407','408','409','410','412','413','414','415','417','419','423','424','425','430','432','434','435','440','442','443','447','448','458','463','464','469','470','475','478','479','480','484','501','502','503','504','505','507','508','509','510','512','513','515','516','517','518','520','530','531','534','539','540','541','551','559','561','562','563','564','567','570','571','573','574','575','580','582','585','586','601','602','603','605','606','607','608','609','610','612','614','615','616','617','618','619','620','623','626','628','629','630','631','636','641','646','650','651','657','659','660','661','662','667','669','671','678','679','680','681','682','701','702','703','704','706','707','708','712','713','714','715','716','717','718','719','720','724','725','726','727','731','732','734','737','740','743','747','754','757','760','762','763','765','769','770','772','773','774','775','779','781','785','786','801','802','803','804','805','806','808','810','812','813','814','815','816','817','818','820','826','828','830','831','832','838','843','845','847','848','850','854','856','857','858','859','860','862','863','864','865','870','872','878','901','903','904','906','907','908','909','910','912','913','914','915','916','917','918','919','920','925','928','929','931','934','936','937','938','940','941','947','949','951','952','954','956','959','970','971','972','973','975','978','979','980','984','985','989'] },
    { cc:'+44',  country:'GB', name:'United Kingdom', prefixes:['7400','7401','7402','7403','7404','7405','7406','7407','7408','7409','7450','7451','7452','7700','7701','7702','7703','7704','7705','7706','7707','7708','7709','7710','7711','7712','7713','7714','7715','7720','7721','7730','7740','7750','7760','7770','7780','7790','7800','7810','7820','7830','7840','7850','7860','7870','7880','7890','7900','7910','7920','7930','7940','7950','7960','7970','7980','7990'] },
    { cc:'+49',  country:'DE', name:'Germany',        prefixes:['151','152','155','157','159','160','162','163','170','171','172','173','174','175','176','177','178','179'] },
    { cc:'+33',  country:'FR', name:'France',         prefixes:['601','602','603','604','605','606','607','608','609','610','611','612','613','614','615','616','617','618','619','620','621','622','623','624','625'] },
    { cc:'+7',   country:'RU', name:'Russia',         prefixes:['901','902','903','904','905','906','907','908','909','910','911','912','913','914','915','916','917','918','919','920','921','922','923','924','925','926','927','928','929','930'] },
    { cc:'+91',  country:'IN', name:'India',          prefixes:['620','630','640','650','660','670','680','690','700','710','720','730','740','750','760','770','780','790','800','810','820','830','840','850','860','870','880','890','900','910','920','930','940','950','960','970','980','990'] },
    { cc:'+55',  country:'BR', name:'Brazil',         prefixes:['11','12','13','14','15','16','17','18','19','21','22','24','27','28','31','32','33','34','35','37','38','41','42','43','44','45','46','47','48','49','51','53','54','55'] },
    { cc:'+61',  country:'AU', name:'Australia',      prefixes:['400','401','402','403','404','405','406','407','408','409','410','411','412','413','414','415','416','417','418','419','420','421','422','423','424','425','426','427','428','429','430','431','432','433','434','435','436','437','438','439','440','441','450','451','452','457','458','459','460','461','462','467','468','469','470','471','472','473','474','475','476','477','478','479','480','481','482','483','484','485','486','487','488','489','490','491','492','497','498','499'] },
    { cc:'+81',  country:'JP', name:'Japan',          prefixes:['70','80','90'] },
    { cc:'+82',  country:'KR', name:'South Korea',    prefixes:['10','16','17','18','19'] },
    { cc:'+34',  country:'ES', name:'Spain',          prefixes:['600','601','602','603','604','605','606','607','608','609','610','611','612','613','614','615','616','617','618','619','620','621','622','623','624','625','626','627','628','629','630'] },
    { cc:'+39',  country:'IT', name:'Italy',          prefixes:['320','328','329','333','334','335','336','337','338','339','340','345','347','348','349','360','366','368','370','380'] },
    { cc:'+31',  country:'NL', name:'Netherlands',    prefixes:['6100','6200','6300','6400','6500','6600','6700','6800','6900'] },
    { cc:'+46',  country:'SE', name:'Sweden',         prefixes:['70','72','73','76','79'] },
    { cc:'+47',  country:'NO', name:'Norway',         prefixes:['40','41','42','45','46','47','48','49','90','91','92','93','94','95','96','97','98','99'] },
    { cc:'+45',  country:'DK', name:'Denmark',        prefixes:['20','22','23','25','26','27','28','29','30','31','40','41','42','50','51','52','53','60','61','71','81'] },
    { cc:'+41',  country:'CH', name:'Switzerland',    prefixes:['74','75','76','77','78','79'] },
    { cc:'+43',  country:'AT', name:'Austria',        prefixes:['650','660','664','676','680','681','699'] },
    { cc:'+52',  country:'MX', name:'Mexico',         prefixes:['55','33','81','222','477','442','229','999'] },
    { cc:'+1',   country:'CA', name:'Canada',         prefixes:['204','226','236','249','250','289','306','343','403','416','418','431','437','438','450','506','514','519','548','587','604','613','639','647','705','709','778','780','807','819','825','867','873','902','905'] },
  ]

  const servicePatterns = [
    'Your Google verification code is ##',
    'Your WhatsApp code: ## - Don\'t share it',
    '[Telegram] ## is your login code',
    'Your Facebook code is: ##',
    'Use ## to verify your Instagram account',
    'Your Twitter confirmation code: ##',
    'Amazon: ## is your OTP. Never share it.',
    'Your Snapchat code is: ##',
    'LinkedIn verification code: ##',
    'Your TikTok code is ##',
    'PayPal Security Code: ##',
    'Your Apple ID code is: ##',
    'Discord: ##',
    'Uber: ##',
    'Your Airbnb code: ##',
  ...
  const perPool = Math.ceil(totalTarget / pools.length)

  for (const pool of pools) {
    const poolCount = Math.min(perPool, totalTarget - numbers.length)
    for (let i = 0; i < poolCount; i++) {
      const prefix = pool.prefixes[rng(pool.prefixes.length)]
      const suffix = String(rng(90000000) + 10000000).slice(0, 8 - prefix.length)
      const phone  = pool.cc + prefix + suffix
      const smsCount  = rng(120)
      const lastHrsAgo = rng(72)
      const hasOtp    = smsCount > 0
      const otp       = hasOtp ? String(rng(900000) + 100000) : null
      const svc       = servicePatterns[rng(servicePatterns.length)]
      const body      = otp ? svc.replace('##', otp) : svc.replace('##', String(rng(900000)+100000))

      numbers.push({
        id:               uuid(),
        user_id:          userId,
        ivasms_id:        String(100000 + numbers.length),
        phone,
        country:          pool.country,
        country_name:     pool.name,
        status:           lastHrsAgo < 48 ? 'active' : 'inactive',
        sms_count:        smsCount,
        last_received:    smsCount > 0 ? new Date(now - lastHrsAgo * 3600000).toISOString() : null,
        last_otp:         otp,
        last_service:     svc.split(' ')[1] || 'Unknown',
        whatsapp_created: rng(3),
        created_at:       new Date(now - rng(30) * 86400000).toISOString(),
        source:           'ivasms',
      })
    }
    if (numbers.length >= totalTarget) break
  }
  return numbers
}

// ─── Seed SMS messages for seeded numbers ──────────────────────────────────
function generateSeedSMS(userId, numbers) {
  const msgs = []
  const now = Date.now()
  const services = [
    { name:'Google',    sender:'Google',       pat: (n) => `Your Google verification code is ${n}. Don't share it with anyone.` },
    { name:'WhatsApp',  sender:'WhatsApp',     pat: (n) => `Your WhatsApp code: ${n}\nDon't share this code with others\n4sgLq1p5sV6` },
    { name:'Telegram',  sender:'Telegram',     pat: (n) => `${n} is your Telegram login code. Do not give this code to anyone.` },
    { name:'Facebook',  sender:'32665',        pat: (n) => `Your Facebook confirmation code is ${n}` },
    { name:'Instagram', sender:'Instagram',    pat: (n) => `Your Instagram confirmation code is ${n}. Don't share it.` },
    { name:'Twitter',   sender:'Twitter',      pat: (n) => `Your Twitter confirmation code is ${n}` },
    { name:'Discord',   sender:'Discord',      pat: (n) => `Your Discord verification code is: ${n}` },
    { name:'TikTok',    sender:'TikTok',       pat: (n) => `[TikTok] ${n} is your verification code. Valid for 5 minutes.` },
    { name:'Amazon',    sender:'Amazon',       pat: (n) => `Amazon: Use this One Time Password: ${n}. Expires in 10 minutes.` },
    { name:'Apple',     sender:'Apple',        pat: (n) => `Your Apple ID code is: ${n}. Do not share it with anyone.` },
    { name:'PayPal',    sender:'PayPal',       pat: (n) => `PayPal: Your security code is ${n}. It expires in 10 minutes.` },
    { name:'Uber',      sender:'Uber',         pat: (n) => `${n} is your Uber code. Never share this with anyone.` },
    { name:'Netflix',   sender:'Netflix',      pat: (n) => `Your Netflix verification code is ${n}` },
    { name:'LinkedIn',  sender:'LinkedIn',     pat: (n) => `Your LinkedIn verification code is ${n}. This code expires in 10 minutes.` },
    { name:'Coinbase',  sender:'Coinbase',     pat: (n) => `Coinbase 2FA: ${n} is your auth code. Valid for 30 seconds.` },
    { name:'Snapchat',  sender:'Snapchat',     pat: (n) => `Your Snapchat verification code is ${n}. Don't share it.` },
    { name:'Reddit',    sender:'Reddit',       pat: (n) => `Your Reddit verification code is: ${n}` },
    { name:'Microsoft', sender:'Microsoft',    pat: (n) => `${n} is your Microsoft account verification code` },
  ]

  const rng = (n) => Math.floor(Math.random() * n)

  // Generate 3-5 SMS per active number (up to 2000 total)
  for (const num of numbers.slice(0, 400)) {
    if (num.sms_count === 0) continue
    const count = Math.min(num.sms_count, rng(5) + 1)
    for (let i = 0; i < count; i++) {
      const svc = services[rng(services.length)]
      const otp = String(rng(900000) + 100000)
      const body = svc.pat(otp)
      msgs.push({
        id:           uuid(),
        user_id:      userId,
        number_id:    num.id,
        ...
        received_at:  new Date(now - rng(72) * 3600000 - rng(3600000)).toISOString(),
        starred:      rng(10) > 8,
        tags:         [],
        read:         rng(3) > 0,
      })
