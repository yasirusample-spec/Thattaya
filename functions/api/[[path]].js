...
// WebCrypto JWT · KV/in-memory store · iVASMS scraper
...
export async function onRequest(context) {
  ...
  try {
    ...
    if (path === '/api/ivasms/numbers' && method === 'GET') {
      ...
      // Compute active/inactive based on last_received recency
      const now = Date.now()
      const enriched = (Array.isArray(numbers) ? numbers : []).map(n => {
        ...
        return { ...n, status: isActive ? 'active' : (n.status || 'inactive') }
      })
    ...
    if (path === '/api/ivasms/sms' && method === 'GET') {
      ...
      const since   = url.searchParams.get('since')    || '' // for live poll
      ...
      if (since) {
        ...
        msgs = msgs.filter(m => new Date(m.received_at).getTime() > sinceMs)
      }
    ...
    if (path === '/api/ivasms/sync' && method === 'POST') {
      // Re-fetch latest user from KV to get saved credentials
      const freshUsers = await kvGet(kv, 'users', [])
      const freshUser = (Array.isArray(freshUsers) ? freshUsers : []).find(u => u.id === user.id) || user

      if (!freshUser.ivasms_email || !freshUser.ivasms_password) {
        return json({ error: 'iVASMS credentials not configured. Go to Settings to add your email and password.' }, 400)
      }

      try {
        const result = await scrapeIVASMS(freshUser.ivasms_email, freshUser.ivasms_password, freshUser.id, kv)
        return json(result)
      } catch (err) {
    ...
    if (path === '/api/settings' && method === 'PATCH') {
      ...
      // Always reload users fresh from store so we have latest state
      ...
      if (body.type === 'profile') {
        if (body.name  && body.name.trim())  users[idx].name  = body.name.trim()
        if (body.email && body.email.trim()) users[idx].email = body.email.toLowerCase().trim()

      } else if (body.type === 'ivasms') {
        users[idx].ivasms_email    = (body.email    || '').trim()
        users[idx].ivasms_password = (body.password || '').trim()
        ...
        if (!(await checkPassword(body.old, users[idx].password_hash)))
      ...
      return json({
        ...
        user: {
          ...
          ivasms_email: updatedUser.ivasms_email || '',
          telegram_bot_token: updatedUser.telegram_bot_token || '',
          telegram_chat_id: updatedUser.telegram_chat_id || '',
          mobile_token: updatedUser.mobile_token || '',
          has_whatsapp: updatedUser.has_whatsapp || 0,
    ...
    if (path === '/api/whatsapp/create' && method === 'POST') {
      return json({
        error: 'WhatsApp registration requires Node.js backend with Puppeteer.',
        note: 'Available when running locally with npm run dev.',
    ...
    // ══════════════════════════════════════════════════════════════════════════
    // DL CHAT — group/private messaging
    // ══════════════════════════════════════════════════════════════════════════
    ...
    if (path === '/api/chat/rooms' && method === 'POST') {
      ...
      const room = {
        ...
        type: body.type || 'group', // group | private
        created_by: user.id,
        created_at: new Date().toISOString(),
        members: [user.id],
      }
    ...
    if (path === '/api/chat/messages' && method === 'GET') {
      ...
      if (since) {
        const sinceMs = new Date(since).getTime()
        result = arr.filter(m => new Date(m.created_at).getTime() > sinceMs)
      } else {
        result = arr.slice(-limit)
      }
    ...
    if (path === '/api/chat/messages' && method === 'POST') {
      ...
      if (!body.text && !body.type) return json({ error: 'Message text required' }, 400)
      ...
      const msg = {
        ...
        text: (body.text || '').trim(),
        type: body.type || 'text', // text | system | otp
        otp: body.otp || null,
        created_at: new Date().toISOString(),
      }
...
async function scrapeIVASMS(email, password, userId, kv) {
  ...
  // ── Step 1: Load login page to get CSRF ──
  let csrfToken = ''
  let initCookies = ''
  try {
    const loginPageRes = await fetch(`${BASE}/login`, { headers: baseHeaders })
    ...
    initCookies = parseCookies(setCookie)
  } catch (e) {
  ...
  try {
    ...
    // Accept 302 (redirect after login) or 200
  ...
  // ── Step 3: Fetch portal/numbers ──
  let numbersHtml = ''
  try {
    const numbersRes = await fetch(`${BASE}/portal/numbers`, {
      headers: { ...baseHeaders, Cookie: sessionCookies, Referer: BASE },
      redirect: 'follow',
    })
    // Follow additional redirects manually if needed
    if (numbersRes.url?.includes('/login')) {
      throw new Error('Authentication failed — iVASMS redirected back to login. Check your credentials.')
    }
    numbersHtml = await numbersRes.text()
    if (numbersHtml.includes('<title>') && numbersHtml.match(/<title>[^<]*[Ll]ogin[^<]*<\/title>/)) {
  ...
  // ── Step 5: Fetch SMS for each number (up to 5 to stay within timeout) ──
  let totalSmsAdded = 0
  const existingMessages = await kvGet(kv, `sms_${userId}`, [])
  const msgArr = Array.isArray(existingMessages) ? existingMessages : []
  const existingKeys = new Set(msgArr.map(m => `${m.phone_number}|${m.sender}|${m.body}`))
  ...
  for (const num of numbers.slice(0, 5)) {
    ...
    try {
      const smsRes = await fetch(`${BASE}/portal/numbers/${num.ivasms_id}/sms`, {
        headers: { ...baseHeaders, Cookie: sessionCookies, Referer: `${BASE}/portal/numbers` },
      })
      const smsHtml = await smsRes.text()
      const newMsgs = parseMessages(smsHtml, num, userId)

      for (const msg of newMsgs) {
        const key = `${msg.phone_number}|${msg.sender}|${msg.body}`
        if (!existingKeys.has(key)) {
          existingKeys.add(key)
          msgArr.unshift(msg)
          totalSmsAdded++
          // Update number's sms_count and last_received
...
function parseNumbers(html, userId) {
  ...
  const countryMap = {
    ...
    DE: 'Germany', FR: 'France', RU: 'Russia', IN: 'India', CN: 'China',
    BR: 'Brazil', CA: 'Canada', AU: 'Australia', JP: 'Japan', KR: 'South Korea',
    SE: 'Sweden', NL: 'Netherlands', PL: 'Poland', UA: 'Ukraine', IT: 'Italy',
    ES: 'Spain', MX: 'Mexico', ID: 'Indonesia', PH: 'Philippines', VN: 'Vietnam',
    TH: 'Thailand', TR: 'Turkey', NG: 'Nigeria', ZA: 'South Africa',
  ...
  while ((match = rowRegex.exec(html)) !== null) {
    ...
    // Try multiple patterns to find a phone number
    const phoneMatch =
    ...
    const isActive = /badge-success|status.*active|active.*status/i.test(row)
